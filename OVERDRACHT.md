...
- Sinds 2026-08-27 open voor `minimumRole: 'user'` (zie `src/lib/toolRegistry.js`) — iedereen kan de matcher gebruiken. Het AVG-/Bullhorn-rechten-gesprek met Sam over bredere uitrol is niet gevoerd - deze uitbreiding was een expliciete keuze van de gebruiker om daar niet langer op te wachten. RLS op `matching_runs`/`matching_resultaten` is niet per-gebruiker gescoped — iedereen ziet elkaars runs, niet alleen de eigen (bewust, zelfde keuze).
- Nieuwe tool "Kandidaat Matcher - Gebruik" (`src/pages/tools/MatcherGebruik.jsx`, `minimumRole: 'admin'`) toont wie de matcher gebruikt, hoe vaak, met hoeveel kandidaten en tegen welke kosten — leest rechtstreeks `matching_runs`, geen aparte tabel.
...
- **2026-08-27:** een echt anonimiserings-lek gevonden en gefixt (informele bijnamen in een CV, bv. "Chris" i.p.v. "Christiaan", werden niet herkend) — zie `SESSIELOG-2026-08-27-KANDIDAAT-MATCHER.md` (sectie 3) voor de volledige analyse en het bekende, geaccepteerde restgat. Ook een nieuwe **intake-datumweergave** toegevoegd ("Laatste intake: DD-MM-YYYY" naast elke kandidaat) — de datum wordt vastgelegd door `sync_candidates.py` (kandidaat-ranker-repo) bij het wegschrijven van de intake-tekst, zie sectie 4 van datzelfde sessielog voor de volledige cross-repo-uitleg en de eenmalige herverwerking die de bestaande populatie een datum heeft gegeven.
